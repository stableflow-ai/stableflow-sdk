...
import { EvmWalletProvider, useEvmWalletContext } from './EvmWalletContext';
...
export const metadata = {
  name: 'StableFlow Demo-Hyperliquid',
  description: 'Hyperliquid deposit via @stableflow/hyperliquid',
...
function Content() {
  ...
  const { setWallet } = useEvmWalletContext();
  ...
  const { run: debouncedSync } = useDebounceFn(
    async () => {
      ...
      setWallet({
        ...
        chainId: account.chainId ?? void 0,
        wallet,
        connect: () => {
          openConnectModal?.();
        },
