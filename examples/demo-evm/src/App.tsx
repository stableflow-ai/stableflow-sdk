  ...
  const handleSubmitTransaction = async () => {
    ...
    try {
      if (quote.needApprove) {
        // If from Ethereum
        // the previous approval needs to be revoked before approving a new amount
        if (fromChainConfig.chainName === "Ethereum") {
          ...
          if (Big(allowanceResult.allowance || 0).gt(0)) {
            const approveRes = await fromWallet.wallet.approve({
              contractAddress: quote.quoteParam.fromToken.contractAddress,
              spender: quote.approveSpender,
              amountWei: "0",
              isDetails: true,
  ...
  return (
    <div className="app">
      <header className="app-header">
        <h1>StableFlow Demo-EVM</h1>
        <p>EVM-only cross-chain bridge example</p>
      </header>
