...
export const getAptosTokens = (): TokenConfig[] => {
  return tokens.filter((token) => token.chainType === 'aptos');
};

export const getBridgeTokens = (): TokenConfig[] => {
  return tokens.filter((token) => token.chainType === 'evm' || token.chainType === 'aptos');
};
